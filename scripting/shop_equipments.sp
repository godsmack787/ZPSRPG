...
new bool:b_remove_on_death = true;
...
public Shop_OnConnected()
{
	...
	b_remove_on_death = bool:KvGetNum(kv, "remove_on_death", 1);
	...
	if (KvGotoFirstSubKey(kv))
	{
		do 
		{
			...
			if (_buffer[0])
			{
				...
				Shop_RegisterCategory(_buffer, phrase, "", OnCategoryRegistered, _, OnCategoryDisplay, _);
			}
		}
...
public OnCategoryRegistered(const String:category[], const String:name[], const String:description[])
{
	...
	{
		if (FindStringInArray(hCategories, category) == -1)
		{
			PushArrayString(hCategories, category);
		}
		...
		if (KvGotoFirstSubKey(kv))
		{
			do 
			{
				if (KvGetSectionName(kv, item, sizeof(item)))
				{
					...
					if (pos != -1 && StrEqual(model[pos+1], "mdl", false) && Shop_StartItem(category, item))
					{
						PrecacheModel(model, true);
						Downloader_AddFileToDownloadsTable(model);
						
						KvGetString(kv, "name", model, sizeof(model), item);
						Shop_SetItemInfo(model, "", KvGetNum(kv, "price", 5000), KvGetNum(kv, "sell_price", 2500), Item_Togglable, KvGetNum(kv, "duration", 604800));
...
public bool:OnCategoryDisplay(client, const String:category[], const String:name[], String:buffer[], maxlength)
{
	new bool:result = false;
	KvRewind(kv);
	if (KvJumpToKey(kv, category))
	{
		KvGetString(kv, sClLang[client], buffer, maxlength);
		if (buffer[0])
		{
			result = true;
		}
	...
	if (KvJumpToKey(kv, category) && KvJumpToKey(kv, item))
	{
		...
		if (KvJumpToKey(kv, "classes"))
		{
			if (KvJumpToKey(kv, buffer))
			{
				...
				KvGetString(kv, "alt_attachment", alt_attachment, sizeof(alt_attachment), "primary");
